...
![USS Enterprise NCC-1701](images/enterprise_lit.jpeg)
...
The Tomy Enterprise has two capacitive touch buttons on its base (A = Mode Select, B = Fire Control). Each button has a small sub-board containing a touch IC. The **copper electrode plate on the back** of each sub-board is the raw capacitive sensing element.
...
> ⚠️ **Critical:** Connect to the **copper electrode plate on the back of the sub-board** — not to the S/G/V signal pads on the front. The signal pads are outputs from the touch IC and cannot be driven externally. This was the key hardware discovery for this project.

![Electrode sub-board showing copper plate on back](images/electrode_subboard.jpeg)
...
2. Type the following and press Enter:
   ```
   W YourWiFiName YourWiFiPassword
   ```
...
> Choose a thin or flat USB-C cable. It must fit alongside the Enterprise's own power cable without blocking the port.

![Main PCB and internal wiring](images/main_pcb.jpeg)

### Step 3 — Prepare the Touch Button Electrode Wires

The two touch button sub-boards are held down by a bracket. Each has a large copper electrode plate on its back face (the side facing inward toward the housing).

![Both electrode sub-boards with wires attached](images/electrode_wiring.jpeg)
...
3. Carefully route each wire out along the side edge of the sub-board
...
![Buttons reinstalled with wires routing out](images/buttons_installed.jpeg)
...
2. Verify the Serial Monitor shows **ENTERPRISE ONLINE** and your IP address
3. Mount the Enterprise onto the base
4. Plug the Enterprise's own USB-C power cable in
5. Test the physical touch buttons — they should still work normally
6. Test the Web UI at `http://<your-IP>:8080`
7. Pair with HomeKit using code **836-17-294**
